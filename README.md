...
## Conclusion
  - ### 결과
    - 본 아이디어는 Mediapipe 기반 손동작 인식 + LSTM 단어 인식 + T5 문장 생성 + 번역 API를 통합하여 문맥 단위 실시간 번역을 구현
    - 별도 하드웨어 없이 스마트폰 카메라만으로 동작하는 점에서 접근성 측면의 신규성 확보
    - 단순한 센서/모델의 나열이 아니라, 단어→문장 생성→번역으로 이어지는 모델 간 상호작용 파이프라인으로 품질을 고도화
    - 무료 모바일 앱으로 제공되어 사회적 확장성 동반
